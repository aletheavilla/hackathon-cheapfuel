...
function AddressAutocomplete({ onAddressSelect, placeholder = "Enter your address..." }) {
  ...
  useEffect(() => {
    console.log('AddressAutocomplete: isLoaded=', isLoaded, 'inputRef=', !!inputRef.current);
    ...
    <div style={{ position: 'relative', width: '100%' }}>
      <input
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === 'Enter') {
            e.preventDefault();
            console.log('AddressAutocomplete: Enter pressed - use dropdown to select!');
          }
        }}
      />
      {inputRef.current?.value && (
      ...
      <p style={{
        ...
        💡 Tip: Start typing, then <strong>select from dropdown</strong> (don't just press Enter)
      </p>
    </div>
  );
}
