        ...
        if response.status_code == 200:
            if data.get('status') == 'OK':
                ...
                if element['status'] == 'OK':
                    ...
                    print_status(f"API returned element status: {element['status']}", 'error')
                    return False
            elif data.get('status') == 'REQUEST_DENIED':
                print_status(f"Request denied: {data.get('error_message', 'Unknown error')}", 'error')
                print_status("Check if Distance Matrix API is enabled in Google Cloud Console", 'warning')
                return False
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            if data.get('status') in ['OK', 'ZERO_RESULTS']:
                ...
                print_status(f"Request denied: {data.get('error_message', 'Unknown error')}", 'error')
                print_status("Check if Places API is enabled in Google Cloud Console", 'warning')
                return False
            else:
                print_status(f"API returned status: {data.get('status')}", 'error')
                if 'error_message' in data:
...
def test_maps_embed_api(api_key):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_status(f"Maps Embed API returned status {response.status_code}", 'warning')
...
def main():
    ...
    if passed_tests == total_tests:
        ...
        print("  • Distance Matrix API: https://console.cloud.google.com/apis/library/distance-matrix-backend.googleapis.com")
        print("  • Places API: https://console.cloud.google.com/apis/library/places-backend.googleapis.com")
        print("  • Maps Embed API: https://console.cloud.google.com/apis/library/maps-embed-backend.googleapis.com")
        sys.exit(1)
    else:
