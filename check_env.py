...
def check_python_version():
    ...
    if version.major >= 3 and version.minor >= 8:
        print_status(
            f"Python {version.major}.{version.minor}.{version.micro} installed",
            "success",
        )
        return True
    else:
        print_status(
            f"Python {version.major}.{version.minor} found, but 3.8+ required", "error"
        )
        return False
...
def check_env_file():
    ...
    if not env_path.exists():
        ...
        print_status(
            "Create backend/.env with GOOGLE_MAPS_API_KEY and SECRET_KEY", "info"
        )
        return False
